...

{
    if($number>0)
    {
      echo "<script>alert('Cant delete registration has already begun');document.location='sportstaff.php'</script>";
    ...
    $item = $_POST['Category'];
    $sql = "ALTER TABLE regsports DROP COLUMN IF EXISTS $item;";
    $result=mysqli_query($conn,$sql);
    ...
    echo "<script>alert('event deleted');document.location='sportstaff.php'</script>";
    }
