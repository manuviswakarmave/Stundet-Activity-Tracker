...

{
    ...
    $sql="SELECT * FROM eventlist WHERE itemname='$ename' AND eventday='sd'";
    ...
    if($number>0)
    {
      echo "<script>alert('registration has already begun');document.location='ads.php'</script>";
        ...
        $sql = "INSERT INTO eventlist(eventday,itemname,rule1,rule2,rule3,rule4) VALUES ('sd','$ename','$rule1','$rule2','$rule3','$rule4')";
        $result=mysqli_query($conn,$sql);
        
        $sql="ALTER TABLE regsports ADD $ename CHAR(3) ;";
        $result=mysqli_query($conn,$sql);
        echo "<script>alert('event added');document.location='sportstaff.php'</script>";
