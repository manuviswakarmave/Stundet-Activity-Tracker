...
if(isset($_POST['submit']))
{
  ...
  if($num > 0)
  {
    echo "<script>alert('dates are already set please reset them');document.location='ads.php'</script>";
  }
  elseif($day<$deadline)
  {
    echo "<script>alert('deadline must be set before the event day');document.location='ads.php'</script>";
  }
  elseif($num1<=0)
  {
    echo "<script>alert('Publish some events before declaring dates');document.location='ads.php'</script>";
  }
  else
  {
   
    $sql = "INSERT INTO eventday ( eid, 
                day, deadline) VALUES ('ad', 
                '$day', '$deadline')";
       $result=mysqli_query($conn,$sql);         
       echo "<script>alert('dates are published');document.location='ads.php'</script>";
...
if(isset($_POST['reset']))
{
  ...
  $result=mysqli_query($conn,$sql);
  $sql = "TRUNCATE TABLE regarts;";
  $result=mysqli_query($conn,$sql);
  echo "<script>alert('dates are reset');document.location='ads.php'</script>";

}

$sql="SELECT * FROM regarts WHERE house='green'; ";
$result=mysqli_query($conn,$sql);
$green=mysqli_num_rows($result);

$sql="SELECT * FROM regarts WHERE house='red'; ";
$result=mysqli_query($conn,$sql);
...
$sql="SELECT * FROM regarts WHERE house='yellow'; ";
$result=mysqli_query($conn,$sql);
...
$sql="SELECT day , deadline FROM eventday WHERE eid='ad'";
$result=mysqli_query($conn,$sql);
...
<body>
  <div class="sidebar">
    <div class="logo-details">
      ...
      <span class="logo_name">JHALAK</span>
    </div>
      <ul class="nav-links">
        <li>
          <a href="#" class="active">
